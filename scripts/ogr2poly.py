...
def createPolys(inOgr, options):
    ...
    # FIXME: Using materialized views so no metadata
    nativeSRS2bufferSRS = osr.CoordinateTransformation(wgsSRS, #lyr.GetSpatialRef(),
                                                       mercSRS)
    bufferSRS2wgsSRS = osr.CoordinateTransformation(mercSRS, wgsSRS)
    nativeSRS2wgsSRS = osr.CoordinateTransformation(wgsSRS, #lyr.GetSpatialRef(),
                                                    wgsSRS)
    ...
    for feat in lyr:
        if options.fieldName != None:
            ...
            inc += 1
        # FIXME: Remove annoying trailing poly_
        polyName = polyName.replace('poly_','')

        logging.info('Creating ' + polyName + '.poly')
        f = open('Poly/' + polyName + '.poly', 'wt')
        print >>f, polyName
